...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    //   promo_discount_item.add(context.read<PromoCodeProvider>().validateProRes?['data'][0]['discounts'][i]['final_discount']);
    // }
                    ...
                    Selector<CartProvider, bool>(
                      builder: (context, data, child) {
                        return DesignConfiguration.showCircularProgress(
                            data, Theme.of(context).colorScheme.fontColor);
                      },
  ...
  Future<void> _getCart(String save) async {
    ...
    if (isNetworkAvail) {
      try {
        ...
        print('____getCart______${parameter}_____${getCartApi}____');
        apiBaseHelper.postAPICall(getCartApi, parameter).then((getdata) {
          ...
          if (!error) {
            ...
            for(int i=0; i<getdata['data'].length; i++){
              ...
              if(getdata['data'][i]['product_details']?.length != 0 || getdata['data'][i]['product_details'] != null){

                for(int j=0; j<getdata['data'][i]['product_details']?.length; j++){

                  if(getdata['data'][i]['product_details'][j]['variants']?.length != 0 || getdata['data'][i]['product_details'][j]['variants'] != null)
                 {
                   for(int k=0; k<getdata['data'][i]['product_details'][j]['variants']?.length; k++){
                     product_ids.add(getdata['data'][i]['product_details'][j]['variants'][k]['product_id']);
                     product_price.add(double.parse(getdata['data'][i]['product_details'][j]['variants'][k]['special_price']) * int.parse(getdata['data'][i]['product_details'][j]['variants'][k]['cart_count']));
                    // print("product_id_______________${product_ids}");
                   }
                 }
                }
              }
            }
            // print("product_id_______________1${product_ids}");
            // print("product_price_______________1${product_price}");
            //
            // print("product_ids______${getdata['data'][0]['product_details'][0]['variants'][0]['product_id']}");
                  ...
                  children: <Widget>[
                                ...
                                ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return CartListViewLayOut(
                                      ...
                                      prod_id: product_ids, prod_prc: product_price,
                                    );
                                  },
                                ),
                                context
                    ...
                    cartList.isNotEmpty
                        ? SimBtn(
                            size: 0.9,
                            height: 40,
                            borderColor: Theme.of(context).colorScheme.txtColor.withOpacity(0.5),
  ...
  storedValidatePromoResponse(){
    context.read<PromoCodeProvider>().validateProRes;
  }
                  ...
                  children: <Widget>[
                                  ...
                                  ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      return CartListViewLayOut(
                                        index: index,
                                        setState: setStateNow,
                                        saveForLatter: saveForLaterFun, prod_id: product_ids, prod_prc: product_price,
                                      );
                                    },
                                  ),
                                if (context
                                  ...
                                  ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      return SaveLatterIteam(
                                        index: index,
                                        setState: setStateNow,
                                        cartFunc: cartFun, prod_id: product_ids, prod_prc: product_price,
                                      );
                                    },
                                  ),
                              ],
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        ...
                        //           child: InkWell(
                        //             onTap: () async{
                        //               if(context.read<CartProvider>().promoC.text == ""){
                        ...
                        //                     ),
                        //                   ),);
                        //                 print("validateProRes     ${context.read<PromoCodeProvider>().validateProRes}");
                        //               }else{
                        ...
                        //                       product_ids: product_ids,
                        //                       product_price: product_price,
                        //                         promo_code: context.read<CartProvider>().promoC.text
                        //                     ),
                        //                   ),
                        //                 );
                        //                 print("validateProRes     ${context.read<PromoCodeProvider>().validateProRes}");
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Row(
                                  ...
                                  children: [
                                    Text(
                                        getTranslated(context, 'TOTAL_PRICE')!,  style: TextStyle(color: Theme.of(context).colorScheme.txtColor),),
                                    Text(
                                      '${DesignConfiguration.getPriceFormat(context, context.read<CartProvider>().oriPrice)!} ',
                                      style: Theme.of(context)
                                          .textTheme
                                ...
                                Row(
                                  mainAxisAlignment:
                                      MainAxisAlignment.spaceBetween,
                                  children: [
                                    Text("Final Price",  style: TextStyle(color: Theme.of(context).colorScheme.txtColor),),
                                    Text('${(context.read<CartProvider>().oriPrice - context.read<CartProvider>().promoAmt).toStringAsFixed(2)} ',
                                      style: Theme.of(context)
                                          .textTheme
                                          .subtitle1!
                                          .copyWith(
                      ...
                      onBtnSelected: () async {
                        if (context.read<CartProvider>().isPromoLen == false) {
                          if (context.read<CartProvider>().oriPrice > 0) {
                            ...
                            if (isAvailable) {
                              print("___________${context.read<CartProvider>().totalPrice}");
                          ...
                          await context
                              ...
                              .validatePromocode(
                                  ...
                                  price: product_price,
                                  promocode: context.read<CartProvider>().promoC.text,///work
                                  update: setStateNow)
                              .then(
                            (value) {
                              setState(
  ...
  checkout() {
    ...
    for(int i=0; i<cartList.length; i++)
     // total_price += (((double.parse(cartList[i].singleItemNetAmount!)) * double.parse(cartList[i].qty!)) + (((double.parse(cartList[i].singleItemTaxAmount!)) * double.parse(cartList[i].qty!))));
     // total_price += (((double.parse(cartList[i].singleItemNetAmount!)) * double.parse(cartList[i].qty!)) + (cartList[i].singleItemTaxAmount.toString() * cartList[i].qty!);
    ...
    return showModalBottomSheet(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              Text(
                                                ...
                                                '${double.parse((total_price).toStringAsFixed(2)) - context.read<CartProvider>().promoAmt!}',
                                                style: TextStyle(
                                                  color: Theme.of(context)
                                                      .colorScheme
                                                      .fontColor,
                                                    ...
                                                    if (context.read<CartProvider>().selAddress == '' ||
                                                      ...
                                                      Navigator.push(
                                                        ...
                                                        CupertinoPageRoute(
                                                          builder: (BuildContext
                                                                  context) =>
                                                              Payment(update: updateCheckout,msg: msg),
                                                        ),
                                                      );
                                                      context
                                                          .read<CartProvider>()
                                                      ...
                                                      if (tempCartListForTestCondtion[
                                                        ...
                                                        msg = getTranslated(
                                                            context,
                                                            'dateWarning');
                                                        Navigator.push(context, CupertinoPageRoute(builder: (BuildContextcontext) => Payment(update: updateCheckout, msg: msg,),
                                                          ),
                                                        );
                                                      }
                                                      context
      ...
      // context.read<PromoCodeProvider>().validateProRes = null;
      // context.read<CartProvider>().promoC.text="";
      // toast('You need to apply promo code again');
    });
  }
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    if (isNetworkAvail) {
      ...
      try {
        ...
        print('promo_discount_item___ ${promo_discount_item.join(',')}');



        request.fields[USER_ID] = CUR_USERID!;
        request.fields['promo_discount_item'] = promo_discount_item.join(',');
        ...
        request.fields[FINAL_TOTAL] = '${(context.read<CartProvider>().oriPrice - context.read<CartProvider>().promoAmt).toStringAsFixed(2)}';
        ...
        print("place_order_params____________${request.fields}");
            ...
            child: AlertDialog(
              ...
              actions: <Widget>[
                ...
                TextButton(
                  ...
                  onPressed: () {
                    Routes.pop(context);
                    if (context.read<CartProvider>().payMethod == getTranslated(context, 'BANKTRAN')) {
                      bankTransfer();
                    } else {
                      placeOrder('');
                    }
