...
class Search extends StatefulWidget {
  bool? fromDashboard;
  Search({Key? key, this.fromDashboard}) : super(key: key);
...
class _SearchState extends State<Search> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading:
        widget.fromDashboard == true? Container():
        Padding(
          padding: const EdgeInsetsDirectional.only(end: 4.0),
          child: IconButton(
            icon: Icon(Icons.arrow_back_ios_rounded, color: Theme.of(context).colorScheme.fontColor,),
            onPressed: () {
              ...
              context.read<SearchProvider>().productList[index].availability ==
                  ...
                  ? Text(getTranslated(context, 'OUT_OF_STOCK_LBL')!,
                      style: Theme.of(context).textTheme.subtitle2!.copyWith(
                          color: Theme.of(context).colorScheme.redDark, fontWeight: FontWeight.bold))
                  : Container(),
            ],
          ),
        ),
